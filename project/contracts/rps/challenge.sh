...
goal app call --app-id "$APP_ID" -f "$CHALLENGER_ACCOUNT" --app-account "$OPPONENT_ACCOUNT" --app-arg "str:challenge" --app-arg "b64:$CHALLENGE_B64" -o challenge-call.tx

# create wager transaction
goal clerk send -a "$WAGER" -t "$APP_ACCOUNT" -f "$CHALLENGER_ACCOUNT" -o challenge-wager.tx
